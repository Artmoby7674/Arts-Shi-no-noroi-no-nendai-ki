 ...
 *
 * Registration order:
 *  1. Config registration      (modContainer)
 *  2. AttachmentType registry  (mod bus)
 *  3. Network packets          (mod bus  — ModNetwork)
 *  4. Game events              (game bus — FightModeHandler)
 *     including AddReloadListenerEvent for TechniqueRegistry / MovesetRegistry
 *  5. Client-only              (mod bus  — ClientSetup)
...
public class ShinoRoi {
    ...
    public ShinoRoi(IEventBus modEventBus, ModContainer modContainer) {
        // 1. Config — must be registered before any config values are read
        modContainer.registerConfig(Type.COMMON, ModConfig.SPEC);

        // 2. Attachment registry — must come first so PlayerData is available
        ModAttachments.ATTACHMENT_TYPES.register(modEventBus);

        // 3. Network packet registration (mod lifecycle bus)
